...
struct Target {
    Target(int x) {} // Conversion définie par l'utilisateur (via constructeur)
    int addOne(int x) { return x + 1; }
};

Target process(Target t) { std::cout << "Cible : Target (User-defined)\n"; return t; }
double process(double d) { std::cout << "Cible : double (Standard)\n"; return d; }

void compute(int x, double y) {}
void compute(double x, int y) {}

int signal(int x)    { std::cout << "Int\n"; return x; }
char signal(char x)   { std::cout << "Char\n"; return x; }
double signal(double x) { std::cout << "Double\n"; return x; }


int main() {
    // ============== Surcharge avec conversion implicte > Surcharge avec conversion définie par l'utilisateur ==============
    int n = 42;
    auto res1 = process(n); // Choisit double



    /* ============== Démonstration de l'inférence locale, res1 récupère le type double alors qu'il 
    aurait aussi pu récupérer Target, causant une erreur de compilation plus tard =======================*/
    // res1.addOne(5);


    /* ============== Ambiguïté entre plusieurs surcharges avec conversion standard 
    implicite (même niveau) ===================*/
    // compute(1.5, 2.5); // ERREUR : "call to 'compute' is ambiguous"


    // ============== Surcharge avec promotion > Surcharge avec conversion standard ==============
    short s = 10;
    auto res2 = signal(s); // Choisit int
}